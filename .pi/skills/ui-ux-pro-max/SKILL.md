...
**Pi / local install:** this folder *is* the skill root. Do not use `CLAUDE_PLUGIN_ROOT`.

```bash
python3 ".pi/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

User copy: `~/.pi/agent/skills/ui-ux-pro-max/scripts/search.py`.

Claude plugin path (only if that layout exists):
